...
//		new NumeroHelper(numero);
	...
	public void insert_appel(Connection co, String destinateur , String destinataire , int minutes , int secondes) throws Exception {
		PreparedStatement st = null;
		String sql = "insert into appel (destinateur, destinataire, minutes, seconde, date_appel) values (?, ?, ?, ?, now())";
		try {
			st = co.prepareStatement(sql);
			Integer id = Helper.getNextId(co, "client");
			st.setInt(1, new Integer(destinateur).intValue());
			st.setInt(2,new Integer(destinataire).intValue());
			st.setInt(3, minutes);
			st.setInt(4, secondes);
			st.execute();
			co.commit();
		} catch(Exception ex) {
			co.rollback();
