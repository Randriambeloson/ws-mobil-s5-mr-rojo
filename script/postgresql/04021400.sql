/*CREATE DATABASE s5final ETU 001026;
...
create or replace view v_somme_solde_credit as
select client.id, sum(valeur) as valeur , numero from v_solde_credit join client on client.id = v_solde_credit.id
group by client.id , numero;
